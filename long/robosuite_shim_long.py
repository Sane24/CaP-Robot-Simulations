...
class MultiBlockTabletop:
    ...
    # ---- low level ----
    def _pos(self, n): return self.env.sim.data.body_xpos[self.env.obj_body_id[n]].copy()

    def _eef(self):
        sid = self.env.sim.model.site_name2id("gripper0_right_grip_site")
        return self.env.sim.data.site_xpos[sid].copy()

    def _step(self, a): self.env.step(a)

    def _osc_goto(self, target, grip, tol=0.006, max_steps=250, gain=8.0):
...
def build_namespace(env):
    return {"get_obj_names": env.get_obj_names, "get_obj_pos": env.get_obj_pos,
