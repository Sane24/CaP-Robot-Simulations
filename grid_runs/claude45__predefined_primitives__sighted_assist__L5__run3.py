...
# generated=2026-08-05T07:40:45
# prompt_chars=15385
# tokens_in=4926 tokens_out=253
# seconds=4.5
...
for i, (block_name, pos) in enumerate(zip(block_names, corners)):
    say_progress(i + 1, len(block_names), f'placing the {block_name} on {get_corner_name(pos)}')
    put_first_on_second(block_name, pos)
    say_verified(lambda b=block_name, p=pos: is_at(b, p),
                 f'The {block_name} is on {get_corner_name(pos)}.',
                 f'The {block_name} did not end up on {get_corner_name(pos)}.')
