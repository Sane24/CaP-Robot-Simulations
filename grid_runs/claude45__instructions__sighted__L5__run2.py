...
# generated=2026-08-05T06:35:16
# prompt_chars=12791
# tokens_in=4105 tokens_out=226
# seconds=3.4
...
for block_name, corner_pos in zip(block_names, corners):
  ...
  if np.linalg.norm(np.array(actual_pos) - np.array(corner_pos)) < 0.05:
    say(f'Successfully placed {block_name} on {get_corner_name(corner_pos)}')
  else:
    say(f'Failed to place {block_name} on {get_corner_name(corner_pos)}')
