...
if np.linalg.norm(np.array(milk_pos) - np.array(milk_bin_pos)) < 0.05:
  say('Success - milk is now in the milk bin')
else:
  say('Failed - milk did not end up in the milk bin')
...
if np.linalg.norm(np.array(bread_pos) - np.array(bread_bin_pos)) < 0.05:
  say('Success - bread is now in the bread bin')
else:
  say('Failed - bread did not end up in the bread bin')
...
if np.linalg.norm(np.array(cereal_pos) - np.array(cereal_bin_pos)) < 0.05:
  say('Success - cereal is now in the cereal bin')
else:
  say('Failed - cereal did not end up in the cereal bin')
...
if np.linalg.norm(np.array(can_pos) - np.array(can_bin_pos)) < 0.05:
  say('Success - can is now in the can bin')
else:
  say('Failed - can did not end up in the can bin')
